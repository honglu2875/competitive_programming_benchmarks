...
double solve(complex<double> a, complex<double> b, complex<double> c, double r) {
    if (r == 0) return abs(a-c) + abs(b-c);

    bool a_in = abs(c-a) <= r;
    bool b_in = abs(c-b) <= r;
    if (a_in || b_in) return abs(a-b);
    if (a == b) return abs(a-c) + abs(b-c) - 2*r;

    auto o = b-a;
    auto pc = c-a;
    double proj = (pc.real()*o.real()+pc.imag()*o.imag())/norm(o);
    double dist = abs(pc.real()*o.imag() - pc.imag()*o.real())/abs(o);
    if (0 <= proj && proj <= 1 && dist < r) return abs(a-b);
    ...
    auto a4 = conj(p);
    auto a3 = -conj(s);
    auto a2 = complex<double>(0);
    auto a1 = s;
    auto a0 = -p;

    constexpr int degree = 4;
    double coeff_real[degree+1] = {a4.real(), a3.real(), a2.real(), a1.real(), a0.real()};
    double coeff_imag[degree+1] = {a4.imag(), a3.imag(), a2.imag(), a1.imag(), a0.imag()};
    double sol_real[degree] = {0};
    double sol_imag[degree] = {0};

    int err = cpoly(coeff_real, coeff_imag, /*degree*/degree, &sol_real[0], &sol_imag[0]);
    assert(err != -1);

    double mind = 1.0/0.0;
    for (int i = 0; i < degree; ++i) {
        complex<double> u(sol_real[i], sol_imag[i]);
        double on_circle = abs(abs(u)-1.0);
        if (on_circle < 1e-7) {
            double d = abs(a-u) + abs(b-u);
            d *= r;  // scale back
            if (d < mind) mind = d;
        }
    }
...
int main() {
    ...
    while (t--) {
        ...
        cout << solve({xa, ya}, {xb, yb}, {xc, yc}, r) << endl;
    }
    return 0;
}
 ...
 * Change Record   :
 *
 * VersionAuthor/DateDescription of changes
 * -------  -----------		----------------------
 * 01.01HVE/021101Initial release
 *
